...
INDI backend uses angular acceleration feedback and a control effectiveness model (`G_*`) to incrementally update motor command inputs. It uses the same attitude target generated by the main attitude controller and computes a rate target from angle error plus attitude feedforward.

## How To Use It

The custom controller is enabled by default in SITL. You can test it using PID or INDI backend.
...
2. Run the following command to display backend parameters. These would be under `CC2_` for PID backend or `CC3_` for INDI backend.
